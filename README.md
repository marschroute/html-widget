...
1. delivery_date  
При этом delivery_date не может быть меньше, чем delivery_date по умолчанию и должно быть в массиве possible_dates в случае, если он не пуст (например, для курьерской службы).  
